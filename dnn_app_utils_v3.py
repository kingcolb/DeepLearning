...
def write_image_data(trainOnly = False):
    ...
    if trainOnly == True:
        ...
        '''train_final = prevData["train_set_x"][:]
        isMale_train = prevData["train_set_y"][:]
        '''
        prevData.close()
    ...
    with h5py.File('data.h5', 'w') as hf:
        if trainOnly == False: 
            for i,img in enumerate(images["train"]): 
                ...
                if gender == 'M' :
                    isMale_train = np.append(isMale_train, np.ones((1, 1)), axis = 0)
                else:
                    isMale_train = np.append(isMale_train, np.zeros((1, 1)), axis = 0)
                   
                print("done with image number: " + str(i))
               
        hf.create_dataset(
                    name="train_set_x",
                    data=train_final,
                    shape=(NUM_TRAIN_IMAGES, HEIGHT, WIDTH, 3),
                    maxshape=(NUM_TRAIN_IMAGES, HEIGHT, WIDTH, 3),
                    compression="gzip",
                    compression_opts=9)
        hf.create_dataset(
                    name="train_set_y",
                    data=isMale_train,
                    shape=(1, NUM_TRAIN_IMAGES),
                    maxshape=(1, NUM_TRAIN_IMAGES),
                    compression="gzip",
                    compression_opts=9)    
        ...
        for i, img in enumerate(images["test"]):
                ...
                if gender == 'M' :
                    isMale_test = np.append(isMale_test, np.ones((1, 1)), axis = 0)
                else:
                    isMale_test = np.append(isMale_test, np.zeros((1, 1)), axis = 0)
                
        hf.create_dataset(
                name="test_set_x",
                data=test_final,
                shape=(NUM_TEST_IMAGES, HEIGHT, WIDTH, 3),
                maxshape=(NUM_TEST_IMAGES, HEIGHT, WIDTH, 3),
                compression="gzip",
                compression_opts=9)
        hf.create_dataset(
                name="test_set_y",
                data=isMale_test,
                shape=(1, NUM_TEST_IMAGES),
